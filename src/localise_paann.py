...
def load_normalization_stats(files_dir):
    mean = np.loadtxt(os.path.join(files_dir, "mean_ann_physics.txt")).astype(np.float32)
    std = np.loadtxt(os.path.join(files_dir, "std_ann_physics.txt")).astype(np.float32)
    return mean, std
...
def load_ground_truth(scenario_id, dataset_dir):
    """
    Robust Ground Truth Loader.
    Reads ALL '_info.csv' files in the Leaks folder.
    Parses 'Description, Value' format.
    """
    ...
    for f in info_files:
        ...
        try:
            ...
            leak_entry = {
                "node": str(data_map.get("Leak Node", "?")),  
                "start": int(data_map.get("Leak Start", 0)),  
                "end": int(data_map.get("Leak End", 0)),      
...
def normalize_data(df, mean, std):
    """
    Normalizes the dataframe using pre-loaded Mean and Std arrays.
    """
...
NODE_DEGREES = {k: len(v) for k, v in NETWORK_GRAPH.items()}
...
def get_topology_score(node_id, all_z_scores, active_baseline_names):
    try:
        ...
        deg = NODE_DEGREES.get(str(node_id), 1)
        degree_mult = 1.0 + (0.05 * deg) 
        
        return own_z * (1.0 + support_ratio) * degree_mult
    except:
        return 0.0

def run_realtime_dashboard(full_data, pressure_data, model, device):
    print(f"--- INITIALIZING PHYSICS-AWARE MONITORING (ANTI-DRIFT) ---")
    
    ann_window = deque(maxlen=WINDOW_SIZE)
    stats_buffer = deque(maxlen=5000)
    detected_history = []; prob_history = []
    
    
    CONFIDENCE_TRIGGER = 0.75  
    CONFIDENCE_RESET = 0.40    
    ...
    for t in range(len(full_data)):
        ...
        if len(ann_window) == WINDOW_SIZE:
            ...
            with torch.no_grad():
                prob_tensor, _, _ = model(inp)
                raw_prob = prob_tensor.item()
            
            if not leak_active: stats_buffer.append(p_row.values)
        ...
        if leak_active and active_baseline:
            ...
            time_weight = 1.0 / (1.0 + 0.1 * duration) 
            ...
            if not is_locked:
                ...
                if sorted_votes:
                    ...
                    if new_winner == current_winner:
                        ...
                        if winner_stability_counter > 10:
                            is_locked = True
                            leak_type = "LOCKED"
                    else:
...
model = LeakPhysicsANN(ann_norm.shape[1] * WINDOW_SIZE).to(device)
        ...
        if leak_active and active_baseline:
            ...
            if is_locked:
                ...
                if sorted_votes:
                    ...
                    if new_winner == current_winner:
                        winner_stability_counter += 1
                        if winner_stability_counter > 10: is_locked = True
                    else:
...
def clean_id(node_id):
    """Normalize node IDs to strings and strip whitespace/prefixes."""
...
for sc_id in tqdm.tqdm(TEST_SCENARIOS):
    ...
    if gt_leaks:
        for leak in gt_leaks:
            ...
            gt_node_raw = leak['node']
            gt_node = clean_id(gt_node_raw)
            
            raw_neighbors = NETWORK_GRAPH.get(gt_node, [])
            valid_set = {gt_node}
            for n in raw_neighbors:
                valid_set.add(clean_id(n))
...
if ALL_TTD:
    ...
    print("Avg Time to Localize:   N/A")
print("-" * 45)
...
if ann_df is not None:
    ...
    plt.plot(time_steps, probs, color="#2c3e50", linewidth=1.5, label="Model Confidence")
    ...
    plt.title(f"Scenario {VIZ_SCENARIO}: PAAN Detection Performance ", fontsize=14)
